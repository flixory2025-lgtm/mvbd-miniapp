...
export default function TelegramJoinPopup({ movieTitle, telegramLink, moviePoster, hasPremiumAccess = false, onClose }: TelegramJoinPopupProps) {
  ...
  return (
    <div className="watch-overlay" role="dialog" aria-modal="true" aria-label="Watch movie" onMouseDown={(event) => { if (event.currentTarget === event.target) onClose() }}>
      <section className="watch-card" onMouseDown={(event) => event.stopPropagation()}>
        ...
        <div className="watch-content">
          ...
          <h2>{movieTitle}</h2>
          {!hasPremiumAccess && <div className="mb-4 rounded-2xl border border-red-300/20 bg-red-300/[.08] p-3 text-sm leading-6 text-red-100"><strong>Premium Subscription Required</strong><p className="mt-1">আপনার active premium subscription নেই। আগে একটি subscription plan নিন, তারপর movieটি দেখতে পারবেন।</p></div>}
          <div className="watch-copy">
            <p>এই movie টি দেখতে হলে আগে নিশ্চিত করুন যে আপনি premium subscription কিনেছেন।</p>
            ...
            <p>Watch Now-তে click করলে আপনাকে Telegram-এর mvbd cloud bot-এ নিয়ে যাবে। সেখানে bot-টি Start করলেই আপনি movie বা series পেয়ে যাবেন।</p>
          </div>
